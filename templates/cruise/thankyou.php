...
<div class="row">
	<div class="col-md-8">
		...
		<div class="step">
			<?php if ( ! empty( $ct_options['cruise_thankyou_text_1'] ) ) : ?>
			<p><?php echo esc_html__( $ct_options['cruise_thankyou_text_1'] , 'citytours' ) ?></p>
			<?php endif; ?>
		</div><!--End step -->
		...
		<div class="step">
			...
			<tr>
				<td><strong><?php echo esc_html__( 'Cruise Name', 'citytours' ) ?></strong></td>
				<td><?php echo get_the_title( $order_data['post_id'] ) ?></td>
			</tr>
			<tr>
				<td><strong><?php echo esc_html__( 'Cabins', 'citytours' ) ?></strong></td>
				<td>
					<?php if ( ! empty( $order_rooms ) ) {
						foreach ($order_rooms as $room ) {
							echo esc_html( $room['cabins'] . ' ' . get_the_title( $room['cabin_type_id'] ) ) . '<br>';
						}
					}?>
				</td>
			</tr>
			<tr>
				<td><strong><?php echo esc_html__( 'Itinerary', 'citytours' ) ?></strong></td>
				<td><?php 
						if ( ct_get_day_interval( $order_data['date_from'], $order_data['date_to']) == '1' ) {
								echo '2 days 1 night';
						} elseif ( ct_get_day_interval( $order_data['date_from'], $order_data['date_to']) == '2' ) {
