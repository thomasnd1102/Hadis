...
// validation
$required_params = array( 'hotel_id', 'date_from', 'itinerary' );
...
$date_to = date('m/d/Y', ct_strtotime( $date_from .' + ' . $itinerary . ' days' ) );
$room_ids = ct_cruise_get_available_rooms( $hotel_id, $date_from, $itinerary );
...
<?php 
	...
	<form id="hotel-cart" action="<?php echo esc_url( add_query_arg( array('uid'=> $uid), ct_get_cruise_checkout_page() ) ); ?>">
		<div class="row">
			<div class="col-md-8">
				<?php do_action( 'hotel_cart_main_before' ); ?>
				<div class="alert alert-danger" role="alert"><?php echo __( '<strong>Cabins available</strong> for the selected dates.<br>PLEASE SELECT YOUR QUANTITY OF CABIN AND THE NUMBER OF ADULTS AND KIDS OF YOUR GROUP AND CLICK UPDATE CART', 'citytours' ) ?></div>
				<table class="table table-striped cart-list hotel add_bottom_30">
					<thead><tr>
						<th><?php echo esc_html__( 'Cabin Type', 'citytours' ) ?></th>
					...
					<tbody>
						<?php foreach ( $room_ids as $room_id => $available_rooms ) :
							$max_adults = get_post_meta( $room_id, '_cabin_max_adults', true );
							$max_kids = get_post_meta( $room_id, '_cabin_max_kids', true );
					...
					<strong><?php echo esc_html__( '* Prices per person.', 'citytours' ) ?></strong><br/>
					<strong><?php echo esc_html__( '** Prices per person including child.', 'citytours' ) ?></strong>    
					<?php endif; ?>
						...
						<tr>
							...
							<td class="text-right">
								<?php 
									  switch ($itinerary) {
									  	case "1" :
									  		echo esc_html__( '2 days 1 night', 'citytours'); break; 
									  	case "2" :
									  		echo esc_html__( '3 days 2 nights', 'citytours'); break;
									  }
								?>
							</td>
						</tr>
	...
	<script>
		...
		$('.update-cart-btn').click(function(e){
			e.preventDefault();
			$('input[name="action"]').val('ct_cruise_update_cart');
			$('#overlay').fadeIn();
			$.ajax({
				url: ajaxurl,
				type: "POST",
