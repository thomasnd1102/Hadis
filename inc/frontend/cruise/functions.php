...
if ( ! function_exists( 'ct_get_cruise_cart_page' ) ) {
	function ct_get_cruise_cart_page() {
		global $ct_options;
		if ( ! empty( $ct_options['cruise_cart_page'] ) ) {
			return ct_get_permalink_clang( $ct_options['cruise_cart_page'] );
		}
		return false;
	}
}
if ( ! function_exists( 'ct_get_cruise_checkout_page' ) ) {
	function ct_get_cruise_checkout_page() {
		global $ct_options;
		if ( ! empty( $ct_options['cruise_checkout_page'] ) ) {
			return ct_get_permalink_clang( $ct_options['cruise_checkout_page'] );
		}
		return false;
	}
}
if ( ! function_exists( 'ct_get_cruise_thankyou_page' ) ) {
	function ct_get_cruise_thankyou_page() {
		global $ct_options;
		if ( ! empty( $ct_options['cruise_thankyou_page'] ) ) {
			return ct_get_permalink_clang( $ct_options['cruise_thankyou_page'] );
		}
		return false;
	}
}
 ...
 */
if ( ! function_exists( 'ct_cruise_get_available_rooms' ) ) {
	function ct_cruise_get_available_rooms( $hotel_id, $from_date, $itinerary, $adults=1, $kids=0 ) {
		//$to_date = ct_strtotime( $from_date .' + ' . $itinerary . ' days' );
		// validation
		if ( empty( $hotel_id ) || 'cruise' != get_post_type( $hotel_id ) ) return esc_html__( 'Invalide Hotel ID.', 'citytours' ); //invalid data
		$hotel_id = esc_sql( ct_hotel_org_id( $hotel_id ) );
		if ( ! ct_strtotime( $from_date ) || ! ct_strtotime( $from_date .' + ' . $itinerary . ' days' )) {
		...
		$sql = "SELECT pm0.post_id FROM " . $wpdb->postmeta . " as pm0 INNER JOIN " . $wpdb->posts . " AS room ON (pm0.post_id = room.ID) AND (room.post_status = 'publish') AND (room.post_type = 'cabin_type') WHERE meta_key = '_cabin_cruise_id' AND meta_value = " . esc_sql( $hotel_id );
		...
		$to_date_obj = new DateTime( '@' . ct_strtotime( $from_date .' + ' . $itinerary . ' days' ) );
		...
		foreach ( $period as $dt ) {
			...
			$sql = "SELECT vacancies.cabin_type_id, vacancies.cabins - IFNULL(bookings.cabins,0) as available_rooms
					FROM (SELECT cabin_type_id, cabins, price_2d1, price_3d2, price_per_child
							FROM " . CT_CRUISE_VACANCIES_TABLE . " 
							WHERE 1=1 AND cruise_id='" . $hotel_id . "' AND cabin_type_id IN (" . implode( ',', $bookable_room_ids ) . ") AND date_from <= '" . $check_date . "'  AND date_to > '" . $check_date . "' ) AS vacancies
					LEFT JOIN (SELECT hotel_booking.cabin_type_id, SUM(hotel_booking.cabins) AS cabins 
							FROM " . CT_CRUISE_BOOKINGS_TABLE . " AS hotel_booking 
							INNER JOIN " . CT_ORDER_TABLE . " as hotel_order on hotel_order.id = hotel_booking.order_id
							WHERE 1=1 AND hotel_order.status!='cancelled' AND hotel_order.post_id='" . $hotel_id . "' AND hotel_order.date_to > '" . $check_date . "'  AND hotel_order.date_from <= '" . $check_date . "'" . " GROUP BY hotel_booking.cabin_type_id
					) AS bookings ON vacancies.cabin_type_id = bookings.cabin_type_id
					WHERE vacancies.cabins - IFNULL(bookings.cabins,0) >= 1;";
					
			
			$results = $wpdb->get_results( $sql ); // object (cabin_type_id, price_2d1, price_3d2, price_per_child, available_rooms)
			...
			foreach ( $results as $result ) {
				$day_on_room_ids[] = $result->cabin_type_id;
				$availability_data[ $result->cabin_type_id ][ $check_date ] = $result->available_rooms;
			}
 ...
 */
if ( ! function_exists( 'ct_cruise_calc_room_price' ) ) {
	function ct_cruise_calc_room_price( $hotel_id, $room_type_id, $from_date, $to_date, $rooms=1, $adults=1, $kids=0 ) {
		global $wpdb;

		$hotel_id = ct_hotel_org_id( $hotel_id );
		$room_type_id = ct_room_org_id( $room_type_id );
		//validation
		$room_hotel_id = get_post_meta( $room_type_id, '_cabin_cruise_id', true );
		if ( $room_hotel_id != $hotel_id ) return esc_html__( 'Room Type Id is not matched.', 'citytours' );

		$max_adults = get_post_meta( $room_type_id, '_cabin_max_adults', true ); if ( empty($max_adults) ) $max_adults = 0;
		$max_kids = get_post_meta( $room_type_id, '_cabin_max_kids', true ); if ( empty($max_adults) ) $max_kids = 0;
		...
		$period = new DatePeriod($from_date_obj, $date_interval, $to_date_obj);
		//mycode
		$itinerary = ct_get_day_interval( $from_date, $to_date );
		//
		...
		foreach ( $period as $dt ) {
			...
			$sql = "SELECT vacancies.cabin_type_id, vacancies.price_2d1 , vacancies.price_3d2, vacancies.price_per_child
					FROM (SELECT cabin_type_id, cabins, price_2d1, price_3d2, price_per_child
							FROM " . CT_CRUISE_VACANCIES_TABLE . " 
							WHERE 1=1 AND cruise_id='" . $hotel_id . "' AND cabin_type_id = '" . $room_type_id . "' AND date_from <= '" . $check_date . "'  AND date_to > '" . $check_date . "' ) AS vacancies
					LEFT JOIN (SELECT hotel_booking.cabin_type_id, SUM(hotel_booking.cabins) AS cabins 
							FROM " . CT_CRUISE_BOOKINGS_TABLE . " AS hotel_booking 
							INNER JOIN " . CT_ORDER_TABLE . " as hotel_order on hotel_order.id = hotel_booking.order_id
							WHERE 1=1 AND hotel_order.status!='cancelled' AND hotel_order.post_id='" . $hotel_id . "' AND hotel_booking.cabin_type_id = '" . $room_type_id . "' AND hotel_order.date_to > '" . $check_date . "' AND hotel_order.date_from <= '" . $check_date . "'" . ( ( empty( $except_booking_no ) || empty( $pin_code ) )?"":( " AND NOT ( hotel_order.booking_no = '" . $except_booking_no . "' AND hotel_order.pin_code = '" . $pin_code . "' )" ) ) . "
					) AS bookings ON vacancies.cabin_type_id = bookings.cabin_type_id
					WHERE vacancies.cabins - IFNULL(bookings.cabins,0) >= " . $rooms . ";";
...
if ( ! function_exists( 'ct_cruise_get_search_result_count' ) ) {
	function ct_cruise_get_search_result_count( $args ) {
		...
		if ( $by == 'star' ) {
			...
			$sql = "SELECT IFNULL( meta_star.meta_value, 0 ) as star, COUNT(*) AS counts FROM {$temp_tbl_name} as t1
					LEFT JOIN {$tbl_postmeta} AS meta_star ON t1.hotel_id = meta_star.post_id AND meta_star.meta_key = '_cruise_star'";
			$where = ' 1=1';
			...
			if ( ! empty( $price_filter ) && trim( implode( '', $price_filter ) ) != "" ) {
				$sql .= " LEFT JOIN {$tbl_postmeta} AS meta_price ON t1.hotel_id = meta_price.post_id AND meta_price.meta_key = '_cruise_price'";
				$price_where = array();
				$price_steps = empty( $ct_options['cruise_price_filter_steps'] ) ? '50,80,100' : $ct_options['cruise_price_filter_steps'];
 ...
 * Get search result
 */
if ( ! function_exists( 'ct_cruise_get_search_result' ) ) {
	function ct_cruise_get_search_result( $args ) {
		global $ct_options, $wpdb;
		$s = '';
		$date_from = '';
		$date_to = '';
		...
		$order = '';
		$last_no = 0;
		$per_page = ( isset( $ct_options['cruise_posts'] ) && is_numeric($ct_options['cruise_posts']) )?$ct_options['cruise_posts']:6;
		extract( $args );
		...
		$c_query = "SELECT t1.*, meta_c1.post_id AS room_id, meta_c2.meta_value AS max_adults, meta_c3.meta_value AS max_kids, meta_c4.meta_value AS minimum_stay
					FROM ( {$s_query} ) AS t1
					INNER JOIN {$tbl_postmeta} AS meta_c1 ON (meta_c1.meta_key = '_cabin_cruise_id') AND (t1.hotel_id = meta_c1.meta_value)
					INNER JOIN {$tbl_postmeta} AS meta_c2 ON (meta_c1.post_id = meta_c2.post_id) AND (meta_c2.meta_key='_cabin_max_adults')
					LEFT JOIN {$tbl_postmeta} AS meta_c3 ON (meta_c1.post_id = meta_c3.post_id) AND (meta_c3.meta_key='_cabin_max_kids')
					LEFT JOIN {$tbl_postmeta} AS meta_c4 ON (t1.hotel_id = meta_c4.post_id) AND (meta_c4.meta_key='_cruise_minimum_stay')";
		...
		if ( $from_date_obj && $to_date_obj ) {
			...
			$v_query = "SELECT t3.hotel_id, t3.room_id, t3.max_adults, t3.max_kids, t3.minimum_stay, MIN(rooms) AS min_rooms FROM (
							SELECT t2.*, (IFNULL(vacancies.cabins,0) - IFNULL(SUM(bookings.cabins),0)) AS rooms, check_dates.check_date 
							FROM ({$c_query}) AS t2
							JOIN ( {$sql_check_date} ) AS check_dates
							LEFT JOIN " . CT_CRUISE_VACANCIES_TABLE . " AS vacancies ON (vacancies.cabin_type_id = t2.room_id) AND (vacancies.date_from <= check_dates.check_date AND vacancies.date_to > check_dates.check_date)
							LEFT JOIN ( SELECT hotel_booking.*, hotel_order.date_from, hotel_order.date_to FROM " . CT_CRUISE_BOOKINGS_TABLE . " AS hotel_booking INNER JOIN " . CT_ORDER_TABLE . " as hotel_order ON hotel_order.id = hotel_booking.order_id AND hotel_order.status!='cancelled' ) AS bookings ON (bookings.cabin_type_id = t2.room_id) AND (bookings.date_from <= check_dates.check_date AND bookings.date_to > check_dates.check_date)
		...
		$sql = " FROM {$temp_tbl_name} as t1
				INNER JOIN {$tbl_posts} post_s1 ON (t1.hotel_id = post_s1.ID) AND (post_s1.post_status = 'publish') AND (post_s1.post_type = 'cruise')";
		$where = ' 1=1';
		...
		// price filter
		$sql .= " LEFT JOIN {$tbl_postmeta} AS meta_price ON post_s1.ID = meta_price.post_id AND meta_price.meta_key = '_cruise_price'";
		if ( ! empty( $price_filter ) && trim( implode( '', $price_filter ) ) != "" ) {
			$price_where = array();
			$price_steps = empty( $ct_options['cruise_price_filter_steps'] ) ? '50,80,100' : $ct_options['cruise_price_filter_steps'];
			$step_arr = explode( ',', $price_steps );
		...
		if ( ! empty( $star_filter ) && trim( implode( '', $star_filter ) ) != "" ) {
			$sql .= " LEFT JOIN {$tbl_postmeta} AS meta_star ON post_s1.ID = meta_star.post_id AND meta_star.meta_key = '_cruise_star'";
 ...
 * Get hotels from ids
 */
if ( ! function_exists( 'ct_cruise_get_hotels_from_id' ) ) {
	function ct_cruise_get_hotels_from_id( $ids ) {
 ...
 */
if ( ! function_exists( 'ct_cruise_get_special_hotels' ) ) {
	function ct_cruise_get_special_hotels( $type='latest', $count=6, $exclude_ids, $district=array() ) {
		$args = array(
				'post_type'  => 'cruise',
				'suppress_filters' => 0,
		...
		if ( $type == 'featured'  ) {
			...
			$sql = 'SELECT hotel_id, COUNT(*) AS booking_count FROM ' . CT_CRUISE_BOOKINGS_TABLE . ' AS booking
			...
			if ( count( $popular_hotel ) < $count ) {
				...
				$result = array_merge( $result, ct_cruise_get_special_hotels( 'latest', $count - count( $popular_hotel ), $exclude_ids, $district ) );
			}
			return $result;
		}
	}
