 ...
 */
if ( ! function_exists( 'ct_cruise_update_cart' ) ) {
	function ct_cruise_update_cart() {
		...
		foreach ( $room_ids as $room_id ) :
			if ( ! empty( $_POST['rooms'][$room_id] ) ) :
				...
				$price_data = ct_cruise_calc_room_price( $hotel_id, $room_id, $date_from, $date_to, $rooms, $adults, $kids );
 ...
 */
if ( ! function_exists( 'ct_cruise_submit_booking' ) ) {
	function ct_cruise_submit_booking() {
		...
		if ( $wpdb->insert( CT_ORDER_TABLE, $order_info ) ) {
			...
			if ( ! empty( $cart_data['room'] ) ) {
				foreach ( $cart_data['room'] as $room_id => $room_data ) {
					...
					$room_booking_info['cruise_id'] = $cart_data['hotel_id'];
					$room_booking_info['cabin_type_id'] = $room_id;
					$room_booking_info['cabins'] = $room_data['rooms'];
					...
					$wpdb->insert( CT_CRUISE_BOOKINGS_TABLE, $room_booking_info );
				}
			}
 ...
 */
if ( ! function_exists( 'ct_ajax_cruise_get_hotel_room_list' ) ) {
	function ct_ajax_cruise_get_hotel_room_list() {
		$hotel_id = ( ! empty ( $_POST['hotel_id'] ) ) ? $_POST['hotel_id'] : 0;
		ct_cruise_get_room_list( $hotel_id );
	}
}
