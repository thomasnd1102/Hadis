...
if ( ! class_exists( 'CT_Hotel_Order' ) ) {
	class CT_Hotel_Order {
		...
		public function get_cabins() {
			...
			if ( empty( $this->order_id ) ) return false;
			$hotel_data = $wpdb->get_results( 'SELECT ct_bookings.* FROM ' . CT_ORDER_TABLE . ' AS ct_order 
											INNER JOIN ' . CT_CRUISE_BOOKINGS_TABLE . ' AS ct_bookings ON ct_bookings.order_id = ct_order.id
											WHERE ct_order.id="' . esc_sql( $this->order_id ) . '"', ARRAY_A );
			if ( empty( $hotel_data ) ) return false;
			return $hotel_data;
		}
		...
		public function get_golf_booking() {
			global $wpdb;
			if ( empty( $this->order_id ) ) return false;
			$golf_booking_data = $wpdb->get_row( 'SELECT ct_bookings.* FROM ' . CT_ORDER_TABLE . ' AS ct_order 
											INNER JOIN ' . CT_GOLF_BOOKINGS_TABLE . ' AS ct_bookings ON ct_bookings.order_id = ct_order.id
											WHERE ct_order.id="' . esc_sql( $this->order_id ) . '"', ARRAY_A );
			if ( empty( $golf_booking_data ) ) return false;
			return $golf_booking_data;
		}
		
		public function get_add_hotel_rooms() {
			global $wpdb;
			if ( empty( $this->order_id ) ) return false;
			$hotel_data = $wpdb->get_results( 'SELECT ct_bookings.* FROM ' . CT_ORDER_TABLE . ' AS ct_order 
											INNER JOIN ' . CT_ADD_HOTEL_BOOKINGS_TABLE . ' AS ct_bookings ON ct_bookings.order_id = ct_order.id
											WHERE ct_order.id="' . esc_sql( $this->order_id ) . '"', ARRAY_A );
			if ( empty( $hotel_data ) ) return false;
			return $hotel_data;
		}
		
		public function get_transfer_booking() {
			global $wpdb;
			if ( empty( $this->order_id ) ) return false;
			$golf_booking_data = $wpdb->get_row( 'SELECT ct_bookings.* FROM ' . CT_ORDER_TABLE . ' AS ct_order 
											INNER JOIN ' . CT_TRANSFER_BOOKINGS_TABLE . ' AS ct_bookings ON ct_bookings.order_id = ct_order.id
											WHERE ct_order.id="' . esc_sql( $this->order_id ) . '"', ARRAY_A );
			if ( empty( $golf_booking_data ) ) return false;
			return $golf_booking_data;
		}
