 ...
 */
if ( ! class_exists( 'CT_Golf_Order_List_Table') ) :
class CT_Golf_Order_List_Table extends WP_List_Table {

	function __construct() {
		global $status, $page;
		parent::__construct( array(
			'singular'  => '_order',     //singular name of the listed records
			'plural'    => 'golf_orders',    //plural name of the listed records
	...
	function column_customer_name( $item ) {
		...
		$actions = array(
			'edit'      => '<a href="' . esc_url( sprintf( $link_pattern, sanitize_text_field( $_REQUEST['post_type'] ), 'golf_orders', 'edit', $item['id'] ) ) . '">Edit</a>',
			'delete'    => '<a href="' . esc_url( sprintf( $link_pattern, sanitize_text_field( $_REQUEST['post_type'] ), 'golf_orders', 'delete', $item['id'] . '&_wpnonce=' . wp_create_nonce( 'order_delete' ) ) ) . '">Delete</a>',
		);
		$content = '<a href="' . esc_url( sprintf( $link_pattern, sanitize_text_field( $_REQUEST['post_type'] ), 'golf_orders', 'edit', $item['id'] ) ) . '">' . esc_html( $item['first_name'] . ' ' . $item['last_name'] ) . '</a>';
	...
	function get_columns() {
		$columns = array(
			...
			'tour_name'=> esc_html__( 'Golf Course', 'citytours' ),
	...
	function prepare_items() {
		...
		$where = "1=1";
		$where .= " AND CT_Orders.post_type='golf_course'";
 ...
 */
if ( ! function_exists( 'ct_golf_order_add_menu_items' ) ) {
	function ct_golf_order_add_menu_items() {
		//add tour orders list page
		$page = add_submenu_page( 'edit.php?post_type=golf_course', 'Golf Course Orders', 'Golf Orders', 'manage_options', 'golf_orders', 'ct_golf_order_render_pages' );
		add_action( 'admin_print_scripts-' . $page, 'ct_golf_order_admin_enqueue_scripts' );
	}
}
 ...
 */
if ( ! function_exists( 'ct_golf_order_render_pages' ) ) {
	function ct_golf_order_render_pages() {
		if ( ( ! empty( $_REQUEST['action'] ) ) && ( ( 'add' == $_REQUEST['action'] ) || ( 'edit' == $_REQUEST['action'] ) ) ) {
			ct_golf_order_render_manage_page();
		} elseif ( ( ! empty( $_REQUEST['action'] ) ) && ( 'delete' == $_REQUEST['action'] ) ) {
			ct_golf_order_delete_action();
		} else {
			ct_golf_order_render_list_page();
		}
	}
}
 ...
 */
if ( ! function_exists( 'ct_golf_order_render_list_page' ) ) {
	function ct_golf_order_render_list_page() {
		global $wpdb;
		$ctOrderTable = new CT_Golf_Order_List_Table();
		$ctOrderTable->prepare_items();
		?>

		<div class="wrap">

			<h2>Golf Course Orders <a href="edit.php?post_type=golf_course&amp;page=golf_orders&amp;action=add" class="add-new-h2">Add New</a></h2>
			...
			<a href="edit.php?post_type=golf_course&amp;page=golf_orders" class="button-secondary"><?php echo esc_html__( 'Show All', 'citytours' ) ?></a>
 ...
 */
if ( ! function_exists( 'ct_golf_order_render_manage_page' ) ) {
	function ct_golf_order_render_manage_page() {
		global $wpdb, $ct_options;
		if ( ! empty( $_POST['save'] ) ) {
			ct_golf_order_save_action();
			return;
		}
		...
		if ( 'edit' == $_REQUEST['action'] ) {
			...
			$tour_data = $order->get_golf_booking();
			$golf_data = $order->get_golf_booking();
			$service_data = $order->get_services();
			$room_data = $order->get_add_hotel_rooms();
		...
		<div class="wrap">
			<?php $page_title = ( 'edit' == $_REQUEST['action'] ) ? 'Edit Golf Order<a href="edit.php?post_type=golf_course&amp;page=golf_orders&amp;action=add" class="add-new-h2">Add New</a>' : 'Add New Golf Order'; ?>
			...
			<form method="post" id="order-form" onsubmit="return manage_order_validateForm();" data-message="<?php echo esc_attr( esc_html__( 'Please select a golf course', 'citytours' ) ) ?>">
				...
				<div class="row postbox">
					<div class="one-half">
						...
						<table class="ct_admin_table ct_order_manage_table">
							<tr>
								<th><?php echo esc_html__( 'Golf Course', 'citytours' ) ?></th>
								<td>
							...
							<tr>
								...
								<td><input type="text" name="golf_date" id="date" value="<?php echo esc_attr( $golf_data['golf_date'] ) ?>"></td>
							</tr>
							<tr>
								<th><?php echo esc_html__( 'Players', 'citytours' ) ?></th>
								<td><input type="number" name="players" value="<?php echo esc_attr( $golf_data['players'] ) ?>"></td>
							</tr>
							<tr>
								<th><?php echo esc_html__( 'Time', 'citytours' ) ?></th>
								<td><input type="text" name="time" value="<?php echo esc_attr( $golf_data['time'] ) ?>"></td>
							</tr>
							<tr>
							...
							<tr>
								<th><?php echo esc_html__( 'Add Service', 'citytours' ) ?></th>
								<td><input type="text" name="add_service" value="<?php echo $golf_data['add_service']; ?>"></td>
							</tr>
							<tr>
				...
				<div class="row postbox ct-order-rooms">
					<h3><?php echo esc_html__( 'Order Add Hotels Detail', 'citytours' ) ?></h3>
					<div class="rooms-wrapper">
						<table class="rooms-table">
							<tbody class="clone-wrapper">
								...
								<?php } else { ?>
									<?php foreach ( $room_data as $key=>$room ) : ?>
										<tr class="clone-field">
											...
											<td><?php echo esc_attr( $room['total_price'] ) ?></td>
										</tr>
									<?php endforeach; ?>
								<?php } ?>
							</tbody>
 ...
 * order delete action
 */
if ( ! function_exists( 'ct_golf_order_delete_action' ) ) {
	function ct_golf_order_delete_action() {
		global $wpdb;
		// data validation
		if ( empty( $_REQUEST['order_id'] ) ) {
		...
		INNER JOIN %2$s AS ct_bookings ON ct_order.id = ct_bookings.order_id
		INNER JOIN %3$s AS ct_services ON ct_order.id = ct_services.order_id
		WHERE ct_order.id = %4$s', CT_ORDER_TABLE, CT_GOLF_BOOKINGS_TABLE, CT_ADD_SERVICES_BOOKINGS_TABLE, '%d' );
 ...
 * order save action
 */
if ( ! function_exists( 'ct_golf_order_save_action' ) ) {
	function ct_golf_order_save_action() {
		//validation
		...
		if ( empty( $_POST['post_id'] ) || 'golf_course' != get_post_type( $_POST['post_id'] ) ) {
			print esc_html__( 'Invalide Golf Course ID.', 'citytours' );
			exit;
		}
		...
		if ( empty( $_POST['id'] ) ) {
			...
			$order_data['post_type'] = 'golf_course';
		...
		$sql = 'DELETE FROM ' . CT_GOLF_BOOKINGS_TABLE . ' WHERE order_id=%d';
		$wpdb->query( $wpdb->prepare( $sql, $order_id ) );
		$format = array( '%d', '%s', '%d', '%s', '%f', '%d', '%s' );
		if ( ! empty( $_POST['tour_booking_id'] ) ) {
			$tour_data['id'] = $_POST['tour_booking_id'];
			$format[] = '%d';
		}
		$wpdb->insert( CT_GOLF_BOOKINGS_TABLE, $tour_data, $format ); // add additional services
		...
		wp_redirect( admin_url( 'edit.php?post_type=golf_course&page=golf_orders&action=edit&order_id=' . $order_id . '&updated=true') );
		exit;
	}
}
 ...
 */
if ( ! function_exists( 'ct_golf_order_admin_enqueue_scripts' ) ) {
	function ct_golf_order_admin_enqueue_scripts() {
...
add_action( 'admin_menu', 'ct_golf_order_add_menu_items' );