 ...
 */
if ( ! class_exists( 'CT_Transfer_Order_List_Table') ) :
class CT_Transfer_Order_List_Table extends WP_List_Table {

	function __construct() {
		global $status, $page;
		parent::__construct( array(
			'singular'  => '_order',     //singular name of the listed records
			'plural'    => 'transfer_orders',    //plural name of the listed records
	...
	function column_customer_name( $item ) {
		...
		$actions = array(
			'edit'      => '<a href="' . esc_url( sprintf( $link_pattern, sanitize_text_field( $_REQUEST['post_type'] ), 'transfer_orders', 'edit', $item['id'] ) ) . '">Edit</a>',
			'delete'    => '<a href="' . esc_url( sprintf( $link_pattern, sanitize_text_field( $_REQUEST['post_type'] ), 'transfer_orders', 'delete', $item['id'] . '&_wpnonce=' . wp_create_nonce( 'order_delete' ) ) ) . '">Delete</a>',
		);
		$content = '<a href="' . esc_url( sprintf( $link_pattern, sanitize_text_field( $_REQUEST['post_type'] ), 'transfer_orders', 'edit', $item['id'] ) ) . '">' . esc_html( $item['first_name'] . ' ' . $item['last_name'] ) . '</a>';
	...
	function get_columns() {
		$columns = array(
			...
			'tour_name'=> esc_html__( 'Transfer', 'citytours' ),
	...
	function process_bulk_action() {
		...
		if ( 'bulk_delete'===$this->current_action() ) {
			...
			$wpdb->query( $wpdb->prepare( $sql, $selected_ids ) );
			wp_redirect( admin_url( 'edit.php?post_type=transfer&page=transfer_orders&bulk_update=true&items=' . $how_many) );
		}
	}
	...
	function prepare_items() {
		...
		$where = "1=1";
		$where .= " AND CT_Orders.post_type='transfer'";
 ...
 */
if ( ! function_exists( 'ct_transfer_order_add_menu_items' ) ) {
	function ct_transfer_order_add_menu_items() {
		//add tour orders list page
		$page = add_submenu_page( 'edit.php?post_type=transfer', 'Transfer Orders', 'Transfer Orders', 'manage_options', 'transfer_orders', 'ct_transfer_order_render_pages' );
		add_action( 'admin_print_scripts-' . $page, 'ct_transfer_order_admin_enqueue_scripts' );
	}
}
 ...
 */
if ( ! function_exists( 'ct_transfer_order_render_pages' ) ) {
	function ct_transfer_order_render_pages() {
		if ( ( ! empty( $_REQUEST['action'] ) ) && ( ( 'add' == $_REQUEST['action'] ) || ( 'edit' == $_REQUEST['action'] ) ) ) {
			ct_transfer_order_render_manage_page();
		} elseif ( ( ! empty( $_REQUEST['action'] ) ) && ( 'delete' == $_REQUEST['action'] ) ) {
			ct_transfer_order_delete_action();
		} else {
			ct_transfer_order_render_list_page();
		}
	}
}
 ...
 */
if ( ! function_exists( 'ct_transfer_order_render_list_page' ) ) {
	function ct_transfer_order_render_list_page() {
		global $wpdb;
		$ctOrderTable = new CT_Transfer_Order_List_Table();
		$ctOrderTable->prepare_items();
		?>

		<div class="wrap">

			<h2>Golf Course Orders <a href="edit.php?post_type=transfer&amp;page=transfer_orders&amp;action=add" class="add-new-h2">Add New</a></h2>
			...
			<a href="edit.php?post_type=transfer&amp;page=transfer_orders" class="button-secondary"><?php echo esc_html__( 'Show All', 'citytours' ) ?></a>
 ...
 */
if ( ! function_exists( 'ct_transfer_order_render_manage_page' ) ) {
	function ct_transfer_order_render_manage_page() {
		global $wpdb, $ct_options;
		if ( ! empty( $_POST['save'] ) ) {
			ct_transfer_order_save_action();
			return;
		}
		...
		if ( 'edit' == $_REQUEST['action'] ) {
			...
			$tour_data = $order->get_transfer_booking();
		...
		<div class="wrap">
			<?php $page_title = ( 'edit' == $_REQUEST['action'] ) ? 'Edit Golf Order<a href="edit.php?post_type=transfer&amp;page=transfer_orders&amp;action=add" class="add-new-h2">Add New</a>' : 'Add New Transfer Order'; ?>
			...
			<form method="post" id="order-form" onsubmit="return manage_order_validateForm();" data-message="<?php echo esc_attr( esc_html__( 'Please select a transfer', 'citytours' ) ) ?>">
				...
				<div class="row postbox">
					<div class="one-half">
						...
						<table class="ct_admin_table ct_order_manage_table">
							<tr>
								<th><?php echo esc_html__( 'Transfer', 'citytours' ) ?></th>
								<td>
									<select name="transfer_id" id="post_id">
										<option></option>
										<?php
											$args = array(
													'post_type'         => 'transfer',
							...
							<tr>
								<th><?php echo esc_html__( 'Vehicle', 'citytours' ) ?></th>
								<td><input type="text" name="vehicle" value="<?php echo esc_attr( $tour_data['vehicle'] ) ?>"></td>
							</tr>
							<tr>
								<th><?php echo esc_html__( 'Date', 'citytours' ) ?></th>
								<td><input type="text" name="transfer_date" id="date" value="<?php echo esc_attr( $tour_data['transfer_date'] ) ?>"></td>
							</tr>
							<tr>
								<th><?php echo esc_html__( 'Adults', 'citytours' ) ?></th>
								<td><input type="number" name="adults" value="<?php echo esc_attr( $tour_data['adults'] ) ?>"></td>
							</tr>
							<tr>
								<th><?php echo esc_html__( 'Kids', 'citytours' ) ?></th>
								<td><input type="number" name="kids" value="<?php echo esc_attr( $tour_data['kids'] ) ?>"></td>
							</tr>
							<tr>
								<th><?php echo esc_html__( 'Time', 'citytours' ) ?></th>
								<td><input type="text" name="transfer_time" value="<?php echo esc_attr( $tour_data['transfer_time'] ) ?>"></td>
							</tr>
							<tr>
					...
					<div class="one-half">
						...
						<table  class="ct_admin_table ct_order_manage_table">
							<tr>
								<th><?php echo esc_html__( 'Customer First Name', 'citytours' ) ?></th>
				...
				<a href="edit.php?post_type=transfer&amp;page=transfer_orders" class="button-secondary">Cancel</a>
 ...
 * order delete action
 */
if ( ! function_exists( 'ct_transfer_order_delete_action' ) ) {
	function ct_transfer_order_delete_action() {
		global $wpdb;
		// data validation
		if ( empty( $_REQUEST['order_id'] ) ) {
		...
		INNER JOIN %2$s AS ct_bookings ON ct_order.id = ct_bookings.order_id
		INNER JOIN %3$s AS ct_services ON ct_order.id = ct_services.order_id
		WHERE ct_order.id = %4$s', CT_ORDER_TABLE, CT_TRANSFER_BOOKINGS_TABLE, CT_ADD_SERVICES_BOOKINGS_TABLE, '%d' );
		$wpdb->query( $wpdb->prepare( $sql, $_REQUEST['order_id'] ) );
		wp_redirect( admin_url( 'edit.php?post_type=transfer&page=transfer_orders') );
		exit;
	}
}
 ...
 * order save action
 */
if ( ! function_exists( 'ct_transfer_order_save_action' ) ) {
	function ct_transfer_order_save_action() {
		//validation
		...
		if ( empty( $_POST['transfer_id'] ) || 'transfer' != get_post_type( $_POST['transfer_id'] ) ) {
			print esc_html__( 'Invalide Transfer ID.', 'citytours' );
			exit;
		}
		...
		$sql = 'DELETE FROM ' . CT_TRANSFER_BOOKINGS_TABLE . ' WHERE order_id=%d';
		$wpdb->query( $wpdb->prepare( $sql, $order_id ) );
		$format = array( '%d', '%s', '%s', '%s', '%d', '%d', '%f', '%d' );
		if ( ! empty( $_POST['tour_booking_id'] ) ) {
			$tour_data['id'] = $_POST['tour_booking_id'];
			$format[] = '%d';
		}
		$wpdb->insert( CT_TRANSFER_BOOKINGS_TABLE, $tour_data, $format ); // add additional services


		wp_redirect( admin_url( 'edit.php?post_type=transfer&page=transfer_orders&action=edit&order_id=' . $order_id . '&updated=true') );
		exit;
	}
}
 ...
 */
if ( ! function_exists( 'ct_transfer_order_admin_enqueue_scripts' ) ) {
	function ct_transfer_order_admin_enqueue_scripts() {
...
add_action( 'admin_menu', 'ct_transfer_order_add_menu_items' );