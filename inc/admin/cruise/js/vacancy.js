...
jQuery(document).ready(function($) {
    ...
    $('#cruise_id').change(function(){
        $.ajax({
            ...
            type: "POST",
            data: {
                'action': 'cruise_get_hotel_room_list',
                'hotel_id' : $(this).val()
            },
            success: function(response){
                if ( response ) {
                    var room_type_id = $('#cabin_type_id').val();
                    $('#cabin_type_id').html(response);
                    $('#cabin_type_id').val(room_type_id);
        ...
        $.ajax({
            ...
            success: function(response){
                if ( response ) {
                    ...
                    $('#cabin_type_filter').val(room_type_id);
                    $('#cabin_type_filter').select2({
                        placeholder: "Filter by Cabin Type",
                        allowClear: true,
                        width: "240px",
    ...
    $('#vacancy-filter').click(function(){
        var hotelId = $('#cruise_filter').val();
        var roomTypeId = $('#cabin_type_filter').val();
        var filter_date = $('#date_filter').val();
        var loc_url = 'edit.php?post_type=cruise&page=vacancys';
        if (hotelId) loc_url += '&cruise_id=' + hotelId;
        if (roomTypeId) loc_url += '&cabin_type_id=' + roomTypeId;
