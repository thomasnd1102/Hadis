 ...
 */
if ( ! class_exists( 'CT_Cruise_Order_List_Table') ) :
class CT_Cruise_Order_List_Table extends WP_List_Table {
	...
	function get_columns() {
		$columns = array(
			...
			'hotel_name'=> esc_html__( 'Cruise Name', 'citytours' ),
	...
	function process_bulk_action() {
		...
		if ( 'bulk_delete'===$this->current_action() ) {
			...
			if ( current_user_can( 'manage_options' ) ) {
				...
				WHERE ct_order.id IN (%4$s)', CT_ORDER_TABLE, CT_CRUISE_BOOKINGS_TABLE, CT_ADD_SERVICES_BOOKINGS_TABLE, "$format" );
			} else {
				$sql = sprintf( 'DELETE ct_order, ct_bookings, ct_services FROM %1$s AS ct_order
				INNER JOIN %2$s AS ct_bookings ON ct_order.id = ct_bookings.order_id
				INNER JOIN %3$s AS ct_services ON ct_order.id = ct_services.order_id
				INNER JOIN %5$s as hotel ON ct_order.post_id=hotel.ID
				WHERE ct_order.id IN (%4$s) AND hotel.post_author = %6$d', CT_ORDER_TABLE, CT_CRUISE_BOOKINGS_TABLE, CT_ADD_SERVICES_BOOKINGS_TABLE, "$format", $post_table_name, $current_user_id );
			}
	...
	function prepare_items() {
		...
		$where = "1=1";
		$where .= " AND CT_Orders.post_type='cruise'";
 ...
 */
if ( ! function_exists( 'ct_cruise_order_add_menu_items' ) ) {
	function ct_cruise_order_add_menu_items() {
		//add hotel orders list page
		$page = add_submenu_page( 'edit.php?post_type=cruise', 'Cruise Orders', 'Orders', 'manage_options', 'orders', 'ct_cruise_order_render_pages' );
		add_action( 'admin_print_scripts-' . $page, 'ct_cruise_order_admin_enqueue_scripts' );
	}
}
 ...
 */
if ( ! function_exists( 'ct_cruise_order_render_pages' ) ) {
	function ct_cruise_order_render_pages() {
		if ( ( ! empty( $_REQUEST['action'] ) ) && ( ( 'add' == $_REQUEST['action'] ) || ( 'edit' == $_REQUEST['action'] ) ) ) {
			ct_cruise_order_render_manage_page();
		} elseif ( ( ! empty( $_REQUEST['action'] ) ) && ( 'delete' == $_REQUEST['action'] ) ) {
			ct_cruise_order_delete_action();
		} else {
			ct_cruise_order_render_list_page();
		}
	}
}
 ...
 */
if ( ! function_exists( 'ct_cruise_order_render_list_page' ) ) {
	function ct_cruise_order_render_list_page() {
		global $wpdb;
		$ctOrderTable = new CT_Cruise_Order_List_Table();
		...
		<div class="wrap">
			...
			<h2>Hotel Orders <a href="edit.php?post_type=cruise&amp;page=orders&amp;action=add" class="add-new-h2">Add New</a></h2>
			...
			<input type="button" name="order_filter" id="cruise-order-filter" class="button" value="Filter">
			<a href="edit.php?post_type=cruise&amp;page=orders" class="button-secondary"><?php echo esc_html__( 'Show All', 'citytours' ) ?></a>
 ...
 */
if ( ! function_exists( 'ct_cruise_order_render_manage_page' ) ) {
	function ct_cruise_order_render_manage_page() {
		global $wpdb, $ct_options;
		if ( ! empty( $_POST['save'] ) ) {
			ct_cruise_order_save_action();
			return;
		}
		...
		<div class="wrap">
			<?php $page_title = ( 'edit' == $_REQUEST['action'] ) ? 'Edit Cruise Order<a href="edit.php?post_type=cruise&amp;page=orders&amp;action=add" class="add-new-h2">Add New</a>' : 'Add New Cruise Order'; ?>
			...
			<form method="post" id="order-form" onsubmit="return manage_order_validateForm();" data-message="<?php echo esc_attr( esc_html__( 'Please select a hotel', 'citytours' ) ) ?>">
				...
				<div class="row postbox">
					<div class="one-half">
						...
						<table class="ct_admin_table ct_order_manage_table">
							<tr>
								<th><?php echo esc_html__( 'Cruise', 'citytours' ) ?></th>
								<td>
				...
				<div class="row postbox ct-order-rooms">
					<h3><?php echo esc_html__( 'Order Cabins Detail', 'citytours' ) ?></h3>
						...
						<?php if ( empty( $room_data ) ) { ?>
							<tr class="clone-field">
								<td>
									...
									<?php
										echo ct_cruise_get_room_list( $order_data['post_id'] );
									?>
									</select>
								</td>
							...
							<?php foreach ( $room_data as $key=>$room ) : ?>
								<tr class="clone-field">
									<td>
										...
										<?php
											echo ct_cruise_get_room_list( $order_data['post_id'], $room['cabin_type_id'] );
										?>
										</select>
									</td>
									<td><input type="text" name="room_rooms[<?php echo esc_attr( $key ) ?>]" value="<?php echo esc_attr( $room['cabins'] ) ?>"></td>
 ...
 * order delete action
 */
if ( ! function_exists( 'ct_cruise_order_delete_action' ) ) {
	function ct_cruise_order_delete_action() {
		global $wpdb;
		// data validation
		if ( empty( $_REQUEST['order_id'] ) ) {
		...
		INNER JOIN %2$s AS ct_bookings ON ct_order.id = ct_bookings.order_id
		INNER JOIN %3$s AS ct_services ON ct_order.id = ct_services.order_id
		WHERE ct_order.id = %4$s', CT_ORDER_TABLE, CT_CRUISE_BOOKINGS_TABLE, CT_ADD_SERVICES_BOOKINGS_TABLE, '%d' );
 ...
 * order save action
 */
if ( ! function_exists( 'ct_cruise_order_save_action' ) ) {
	function ct_cruise_order_save_action() {
		//validation
		...
		if ( empty( $_POST['post_id'] ) || 'cruise' != get_post_type( $_POST['post_id'] ) ) {
		...
		if ( ! empty( $_POST['room_post_id'] ) ) {
			...
			$sql = 'DELETE FROM ' . CT_CRUISE_BOOKINGS_TABLE . ' WHERE order_id=%d';
			...
			for ( $index = 0; $index < count( $room_post_id_list ); $index++ ) {
				...
				$wpdb->insert( CT_CRUISE_BOOKINGS_TABLE, $room_data, $format ); // add additional services
			}
		}
		...
		wp_redirect( admin_url( 'edit.php?post_type=cruise&page=orders&action=edit&order_id=' . $order_id . '&updated=true') );
		exit;
	}
}
 ...
 */
if ( ! function_exists( 'ct_cruise_order_admin_enqueue_scripts' ) ) {
	function ct_cruise_order_admin_enqueue_scripts() {
...
add_action( 'admin_menu', 'ct_cruise_order_add_menu_items' );