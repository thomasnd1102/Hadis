		...
		if ( ! empty( $reviews ) ) {
			...
			?>
				<div class="review_strip_single guest-review">
					...
					<img width="76" height="76" alt="avatar" src="https://hanoidiscovery.com/wp-content/uploads/2016/04/avatar.jpg" class="img-circle" data-pin-nopin="true">
...
if ( ! function_exists( 'ct_ajax_get_more_reviews' ) ) {
	function ct_ajax_get_more_reviews() {
		if ( empty( $_POST['post_id'] ) || empty( $_POST['last_no'] ) ) return false;
...
if ( ! function_exists( 'tnd_ajax_get_hotel_room_price' ) ) {
	function tnd_ajax_get_hotel_room_price() {
		...
		if ( ! isset( $_POST['_wpnonce'] ) || ! wp_verify_nonce( $_POST['_wpnonce'], 'get_hotel_room_price' ) ) {
		...
		$total_kids = 0;
		$total_price = 0;
		$cart_data['hotel_name'] = get_the_title( $hotel_id );
		// function
...
if ( ! function_exists( 'tnd_get_hotel_room_list' ) ) {
	function tnd_get_hotel_room_list() {
		...
		if ( ! isset( $_POST['_wpnonce'] ) || ! wp_verify_nonce( $_POST['_wpnonce'], 'get_hotel_room_list' ) ) {
			print esc_html__( 'Sorry, your nonce did not verify.', 'citytours' );
			return false;
		}
		$hotel_id = sanitize_text_field( $_POST['hotel_id'] );
		...
		if ( ! empty( $room_ids ) && is_array( $room_ids ) ) {
			echo '<hr><div class="alert alert-success" role="alert"><strong>Rooms available</strong> for the selected dates.<br>Check in: ' . $date_from . ' - Check out: ' . $date_to . '<br>Nights: ' . ct_get_day_interval( $date_from, $date_to ) . '<br>PLEASE SELECT YOUR QUANTITY.</div>';
			echo '<form id="hotel-cart" action="">';
			wp_nonce_field( 'get_hotel_room_price' ); 
						...
						<tr>
							...
							<td>
								<div class="numbers-row" data-min="0" data-max="<?php echo esc_attr( $available_rooms ) ?>">
									...
									<div class="inc button_inc button_inc_tnd">+</div><div class="dec button_inc button_inc_tnd">-</div>
								</div>
							</td>
							<td>
								<div class="numbers-row" data-min="0" <?php if ( ! empty( $max_adults ) ) echo 'data-max="' . esc_attr( $max_adults * $available_rooms ) . '" data-per-room="' . esc_attr( $max_adults ) . '"'; ?>>
									...
									<div class="inc button_inc button_inc_tnd">+</div><div class="dec button_inc">-</div>
								</div>
							</td>
							<td>
			...
			echo '<h5 class="alert alert-warning">' . esc_html( $room_ids ) . '</h5>';
			exit();
		}
	}
}
/*
 * get add hotel to cart thaond ajax request
 */
if ( ! function_exists( 'tnd_ajax_add_hotel_cart' ) ) {
	function tnd_ajax_add_hotel_cart() {
		if ( empty( $_POST['cart_hotel'] ) ) return false;
		$cart_hotel = $_POST['cart_hotel'];
		//$uid = $golf_id . $cart_hotel['hotel_id'] . $cart_hotel['date_from'] . $cart_hotel['date_to'];
		$uid = mt_rand();
		CT_Hotel_Cart::set( $uid, $cart_hotel );
		//$cart = new CT_Hotel_Cart();
		//echo $cart->get_field( $uid, 'hotel_name' );
		//$cart_data = CT_Hotel_Cart::get( $uid );
		//echo $cart_data['hotel_name'];
 ...
 * render list hotel
 */
if ( ! function_exists( 'tnd_render_list_hotel' ) ) {
	function tnd_render_list_hotel( $related_hotel ) {
		foreach ( $related_hotel as $each_hotel ) {
			...
			$review_item = get_post_meta( $each_hotel, '_review', true );
			echo '<div class="box_style_1 hotel_box" id="' .  $each_hotel . '">';
			echo '<div class="row">';
			echo '<div class="col-md-3 col-xs-6">';
			echo '<a href="' . $hotel_url . '" target="_blank">';
			echo get_the_post_thumbnail( $each_hotel, array(150,150), array( 'class' => 'img-responsive', 'style' => 'margin: auto; display: block; height: 120px;' ));
			echo '</a>';
			echo '</div>';
			echo '<div class="col-md-4 col-xs-6" style="border-right: 1px solid #ededed;">';
			echo '<a href="' . $hotel_url . '" target="_blank"><H5 class="hotel-name show">' . get_the_title( $each_hotel ) . '</h5></a>';
			echo '<span>Hanoi Hotel</span>';
			echo '<br>';
			echo '<span class="">Price per Night from ' . ct_price( $hotel_price ) . '</span>';
			echo '<span class="rating show star" style="font-size: 11px;">';
			for ( $i = 1; $i <= 5; $i++ ) {
				$class = ( $i <= $star_hotel ) ? 'icon-star voted' : 'icon-star-empty';
				echo '<i class="' . $class . '"></i>';
			}
			echo '</span>';
			echo '<span class="rating show review" style="font-size: 11px;">';
			for ( $i = 1; $i <= 5; $i++ ) {
				$class = ( $i <= $review_item ) ? 'icon-smile voted' : 'icon-smile';
				echo '<i class="' . $class . '"></i>';
			}
			echo '</span>';
			echo '</div>';
			echo '<div class="col-md-5 col-xs-12 block-center">';
			echo '<form method="post" action="">';
			wp_nonce_field( 'get_hotel_room_list' );
			echo '<input type="hidden" name="action" value="tnd_get_hotel_room_list">';
			echo '<input type="hidden" name="hotel_id" value="' . $each_hotel . '">';
			echo '<div class="row">';
			echo '<div class="col-md-6 col-sm-6">';
			echo '<div class="form-group">';
			echo '<label><i class="icon-calendar-7"></i> Check in</label>';
			echo '<input class="date-pick form-control" data-date-format="mm/dd/yyyy" type="text" name="date_from">';
			echo '</div>';
			echo '</div>';
			echo '<div class="col-md-6 col-sm-6">';
			echo '<div class="form-group">';
			echo '<label><i class="icon-calendar-7"></i> Check out</label>';
			echo '<input class="date-pick form-control" data-date-format="mm/dd/yyyy" type="text" name="date_to">';
			echo '</div>';
			echo '</div>';
			echo '</div>';
			echo '<a href="<?php echo $hotel_url ?>" target="_blank" class="btn_1 outline btn_full view_room_btn" type="button"><small>View Rooms</small></a>';
			echo '</form>';	
			echo '</div>';
			echo '</div>';
			echo '</div>';
...
/*
 * get hotel class tab ajax request
 */
if ( ! function_exists( 'tnd_hotel_class_tab_ajax' ) ) {
	function tnd_hotel_class_tab_ajax() {
		if ( empty( $_POST['hc'] ) || empty( $_POST['postid'] ) ) return false;
		$hc = $_POST['hc'];
		$postid = $_POST['postid'];
		$related_hotel = get_post_meta( $postid, $hc );
		if ( ! empty( $related_hotel ) ) {
			tnd_render_list_hotel( $related_hotel );
		}
		exit();
	}
}
/*
 * render tour acommodations
 */
if ( ! function_exists( 'tnd_render_accomodation_tour' ) ) {
	function tnd_render_accomodation_tour( $accomodations ) {
		echo '<table class="table"><tbody>';
		foreach ( $accomodations as $each_hotel ) {
			if ( get_post_type( $each_hotel ) == 'hotel' ) {
			    $price = get_post_meta( $each_hotel, '_hotel_price', true );
				$star = get_post_meta( $each_hotel, '_hotel_star', true );
				$brief = get_post_meta( $each_hotel, '_hotel_brief', true ); 
			} elseif ( get_post_type( $each_hotel ) == 'cruise' ) {
				$price = get_post_meta( $each_hotel, '_cruise_price', true );
				$star = get_post_meta( $each_hotel, '_cruise_star', true );
				$brief = get_post_meta( $each_hotel, '_cruise_brief', true ); 
			}
			$hotel_price = get_post_meta( $each_hotel, '_hotel_price', true );
			$star_hotel = get_post_meta( $each_hotel, '_hotel_star', true );
			$hotel_url = esc_url( get_permalink( $each_hotel ));
			$review_item = get_post_meta( $each_hotel, '_review', true );
				...
				echo '<td>';
					echo '<a href="' . $hotel_url . '" target="_blank">';
					echo '<figure>' . get_the_post_thumbnail( $each_hotel, array(45,45), array( 'class' => 'img-rounded' )) . '</figure>';
					echo '</a>';
				echo '</td>';
				echo '<td>';
					echo '<a href="' . $hotel_url . '" target="_blank"><h5 style="font-size: 15px;margin: 0;padding: 0;">' . get_the_title( $each_hotel ). '</h5></a>';
					echo '<div class="show">';
						echo '<span class="rating star" style="font-size: 11px;">';
						for ( $i = 1; $i <= 5; $i++ ) {
							$class = ( $i <= $star ) ? 'icon-star voted' : 'icon-star-empty';
							echo '<i class="' . $class . '"></i>';
						}
						echo '</span>  |  ';
						echo '<span class="rating review" style="font-size: 11px;">';
						for ( $i = 1; $i <= 5; $i++ ) {
							$class = ( $i <= $review_item ) ? 'icon-smile voted' : 'icon-smile';
							echo '<i class="' . $class . '"></i>';
						}
						echo '</span>';
					echo '</div>';
					echo '<small style="padding-top:5px;">' . $brief . '</small>';
				echo '</td>';
				echo '</tr>';
		}
		echo '</tbody></table>';
...
/*
 * get tour accomodation tab ajax request
 */
if ( ! function_exists( 'tnd_tour_accomodation_class_tab_ajax' ) ) {
	function tnd_tour_accomodation_class_tab_ajax() {
		if ( empty( $_POST['hc'] ) || empty( $_POST['postid'] ) ) return false;
		$hc = $_POST['hc'];
		$postid = $_POST['postid'];
		$related_hotel = get_post_meta( $postid, $hc );
		if ( ! empty( $related_hotel ) ) {
			tnd_render_accomodation_tour( $related_hotel );
		}
		exit();
	}
}
...
/*
 * get tour itinerary detail ajax request
 */
if ( ! function_exists( 'tnd_get_tour_itinerary_detail_ajax' ) ) {
	function tnd_get_tour_itinerary_detail_ajax() {
		if ( empty( $_POST['postid'] ) ) return false;
		$postid = $_POST['postid'];
		$itinerary = get_post_meta( $postid, '_tour_itinerary', true );
		if ( ! empty( $itinerary ) ) {
			echo $itinerary ;
		}
		exit();
...
/*
 * render tour form booking
 */
if ( ! function_exists( 'tnd_tour_form_booking' ) ) {
	function tnd_tour_form_booking() {
		$_countries = ct_get_all_countries();
		?>
		<div class="row">
			<div class="col-md-12">
				<div>
					<h5><?php echo esc_html__( 'Your Details', 'citytours' ) ?></h3>
				...
				<div>
					<h5><?php echo esc_html__( 'Your Address', 'citytours' ) ?></h3>
...
/*
 * tour submit booking ajax request
 */
if ( ! function_exists( 'tnd_tour_submit_booking' ) ) {
	function tnd_tour_submit_booking() {
		...
		// init variables
		$post_fields = array( 'first_name', 'last_name', 'email', 'phone', 'country', 'address1', 'address2', 'city', 'state', 'zip', 'post_id' );
		...
		//$post_fields2 = array('date', 'type', 'adults', 'kids', 'post_id', 'total_price');
		...
		$order_info['total_price'] = intval(sanitize_text_field( $_POST[ 'total_price' ] ));
		$order_info['total_adults'] = sanitize_text_field( $_POST[ 'adults' ] );
		$order_info['total_kids'] = sanitize_text_field( $_POST[ 'kids' ] );
		$order_info['other'] = sanitize_text_field( $_POST[ 'type' ] );
		$order_info['status'] = 'new'; // new
		$order_info['deposit_paid'] = 1;
		$order_info['mail_sent'] = 0;
		$order_info['date_from'] = date( 'Y-m-d', ct_strtotime( sanitize_text_field( $_POST[ 'date' ] ) ) );
		...
		if ( $wpdb->insert( CT_ORDER_TABLE, $order_info ) ) {
			$order_id = $wpdb->insert_id;
				...
				$tour_booking_info['tour_id'] = sanitize_text_field( $_POST[ 'post_id' ] );
				$tour_booking_info['tour_date'] = date( 'Y-m-d', ct_strtotime( sanitize_text_field( $_POST[ 'date' ] ) ) );
				$tour_booking_info['adults'] = sanitize_text_field( $_POST[ 'adults' ] );
				$tour_booking_info['kids'] = sanitize_text_field( $_POST[ 'kids' ] );
				$tour_booking_info['total_price'] = intval(sanitize_text_field( $_POST[ 'total_price' ] ));
...
/*
 * golf course submit booking ajax request
 */
if ( ! function_exists( 'tnd_golf_course_submit_booking' ) ) {
	function tnd_golf_course_submit_booking() {
		global $wpdb, $ct_options;
		...
		$order_info['pin_code'] = $pin_code;
		$order_info['created'] = date( 'Y-m-d H:i:s' );
		$order_info['post_type'] = 'golf_course';
		...
		if ( $wpdb->insert( CT_ORDER_TABLE, $order_info ) ) {
			$order_id = $wpdb->insert_id;
				...
				$tour_booking_info = array();
				$tour_booking_info['order_id'] = $order_id;
				$tour_booking_info['golf_id'] = sanitize_text_field( $_POST[ 'post_id' ] );
				$tour_booking_info['golf_date'] = date( 'Y-m-d', ct_strtotime( sanitize_text_field( $_POST[ 'date' ] ) ) );
				$tour_booking_info['players'] = sanitize_text_field( $_POST[ 'players' ] );
				$tour_booking_info['time'] = sanitize_text_field( $_POST[ 'time' ] );
				$tour_booking_info['add_service'] = sanitize_text_field( $_POST[ 'add_service' ] ); 
				$tour_booking_info['total_price'] = intval(sanitize_text_field( $_POST[ 'total_price' ] ));
				$wpdb->insert( CT_GOLF_BOOKINGS_TABLE, $tour_booking_info );
				
				if ( isset( $_POST['add_hotels'] ) ) {
					$add_hotels = array();
					$add_hotels = $_POST['add_hotels'];
					$tour_booking_info['booking_hotel'] = '';
					foreach ( $add_hotels as $key => $uid ) {
						if ( $cart_data = CT_Hotel_Cart::get( $uid ) ) {
							$add_hotel_booking_info = array();
							$add_hotel_booking_info['order_id'] = $order_id;
							$add_hotel_booking_info['total_price'] = $cart_data['total_price'];
							$add_hotel_booking_info['date_from'] = $cart_data['date_from'];
							$add_hotel_booking_info['date_to'] = $cart_data['date_to'];
							$add_hotel_booking_info['hotel_id'] = $cart_data['hotel_id'];
							$rooms = $cart_data['room'];
							foreach ( $rooms as $key => $data ) {
								$add_hotel_booking_info['room_type_id'] = $key;
								$add_hotel_booking_info['rooms'] = $data['rooms'];
								$add_hotel_booking_info['adults'] = $data['adults'];
								$add_hotel_booking_info['kids'] = $data['kids'];
								$add_hotel_booking_info['total_price'] = $data['total'];
								$wpdb->insert( CT_ADD_HOTEL_BOOKINGS_TABLE, $add_hotel_booking_info );
							}
						}
					}
				}
...
/*
 * transfer submit booking ajax request
 */
if ( ! function_exists( 'tnd_transfer_submit_booking' ) ) {
	function tnd_transfer_submit_booking() {
		global $wpdb, $ct_options;
		...
		$order_info['deposit_paid'] = 1;
		$order_info['mail_sent'] = 0;
		$order_info['date_from'] = date( 'Y-m-d', ct_strtotime( sanitize_text_field( $_POST[ 'date_pick' ] ) ) );
		$order_info['booking_no'] = $booking_no;
		$order_info['pin_code'] = $pin_code;
		$order_info['created'] = date( 'Y-m-d H:i:s' );
		$order_info['post_type'] = 'transfer';
		...
		if ( $wpdb->insert( CT_ORDER_TABLE, $order_info ) ) {
			...
			$tour_booking_info = array();
			$tour_booking_info['order_id'] = $order_id;
			$tour_booking_info['transfer_id'] = sanitize_text_field( $_POST[ 'post_id' ] );
			$tour_booking_info['transfer_date'] = date( 'Y-m-d', ct_strtotime( sanitize_text_field( $_POST[ 'date_pick' ] ) ) );
			$tour_booking_info['transfer_time'] = sanitize_text_field( $_POST[ 'time_pick' ] );
			$tour_booking_info['vehicle'] = sanitize_text_field( $_POST[ 'transfer-type' ] );
			$tour_booking_info['adults'] = sanitize_text_field( $_POST[ 'adults' ] );
			$tour_booking_info['kids'] = sanitize_text_field( $_POST[ 'children' ] );
			$tour_booking_info['total_price'] = intval(sanitize_text_field( $_POST[ 'total_price' ] ));
			$wpdb->insert( CT_TRANSFER_BOOKINGS_TABLE, $tour_booking_info );
...
/*
 * tnd transfer generate conf mail 
 */
 
if ( ! function_exists( 'tnd_transfer_generate_conf_mail' ) ) {
	function tnd_transfer_generate_conf_mail( $order, $type='new' ) {
		global $wpdb, $ct_options;
		...
		if ( ! empty( $order_data ) ) {
			...
			$mail_sent = ct_send_mail( 'Hanoi Discovery', 'thaond1986@gmail.com', 'thaond1986@gmail.com', 'Booking Confirmation Email Subject', '' );
 ...
 * get tour itinerary detail ajax request
 */
if ( ! function_exists( 'tnd_render_mobile_end_page_menu' ) ) {
	function tnd_render_mobile_end_page_menu() {
		?>
		<div class="row">
			<div class="col-md-12">
