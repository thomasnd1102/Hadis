...
if ( ! function_exists( 'ct_get_header_image_src' ) ) {
	function ct_get_header_image_src( $post_id = 0 ) {
		...
		} elseif ( 'cruise' == $post_id ) {
			if ( ! empty( $ct_options['cruise_header_img'] ) ) $header_img_scr = $ct_options['cruise_header_img']['url'];	
		} else {
...
if ( ! function_exists( 'ct_get_header_image_height' ) ) {
	function ct_get_header_image_height( $post_id = 0 ) {
		...
		if ( 'blog' == $post_id ) {
			...
			if ( ! empty( $ct_options['cruise_header_img_height'] ) ) $header_img_height = intval( $ct_options['cruise_header_img_height']['height'] );
		} else {
...
if ( ! function_exists( 'ct_enqueue_scripts' ) ) {
	function ct_enqueue_scripts() {
		...
		wp_register_style( 'ct_style_bootstrap', 'https://maxcdn.bootstrapcdn.com/bootstrap/3.3.2/css/bootstrap.min.css' );
		...
		wp_register_style( 'ct_style_responsive', CT_TEMPLATE_DIRECTORY_URI . '/css/responsive.min.css' );
		...
		//enqueue soap icon
		//wp_register_style( 'ct_style_soapicon', CT_TEMPLATE_DIRECTORY_URI . '/css/fontello/css/style-red.css' );
		//wp_enqueue_style( 'ct_style_soapicon');
		// rtl css
		if ( is_rtl() ) {
		...
		wp_register_script( 'ct_script_timepicker', CT_TEMPLATE_DIRECTORY_URI . '/js/bootstrap-timepicker.min.js', array( 'jquery' ), '', true );
		...
		// map
		wp_register_script( 'ct_script_google_map', 'https://maps.googleapis.com/maps/api/js?key=AIzaSyDzTW_X5VP5a03cOfMDHPIYHYe110_hZtM', array(), '', true );
		wp_register_script( 'ct_script_infobox', CT_TEMPLATE_DIRECTORY_URI . '/js/infobox.min.js', array(), '', true );
		wp_register_script( 'ct_script_map', CT_TEMPLATE_DIRECTORY_URI . '/js/map.js', array(), '', true );
		wp_register_script( 'ct_script_gmaps', CT_TEMPLATE_DIRECTORY_URI . '/js/gmaps.min.js', array(), '', true );
		...
		if ( ! empty( $ct_options['restaurant_map_icon'] ) ) { 
			wp_localize_script( 'ct_script_map', 'restaurant_icon', $ct_options['restaurant_map_icon'] );
		}
		...
		//wp_register_script( 'ct_script_webfont', 'https://ajax.googleapis.com/ajax/libs/webfont/1.5.18/webfont.js', array(), '', false );
		
		//wp_enqueue_script( 'ct_script_webfont' );
		...
		if ( is_singular( array('transfer', 'hotel', 'tour', 'cruise', 'restaurant', 'golf_course' ) ) ) {
			wp_enqueue_style( 'ct_style_magnific_popup');
			//wp_enqueue_script( 'ct_script_google_map' );
			//wp_enqueue_script( 'ct_script_map' );
			//wp_enqueue_script( 'ct_script_infobox' );
			...
			//wp_enqueue_script( 'ct_script_gmaps' );
			//wp_enqueue_script( 'ct_script_google_map' );
		}
		if ( is_singular( array('transfer', 'golf_course' ) ) ) {
			wp_enqueue_script( 'ct_script_timepicker' );
		}
		if ( is_post_type_archive('tour') ) {
			wp_enqueue_script( 'ct_cat_nav_mobile' );
		}
		if ( is_singular('hanoi_sight') ) {
			wp_enqueue_style( 'ct_style_magnific_popup');
			//wp_enqueue_script( 'ct_script_google_map' );
			//wp_enqueue_script( 'ct_script_map' );
			//wp_enqueue_script( 'ct_script_infobox' );
		}
		if ( is_archive('hanoi_sight') ) {
			wp_enqueue_script( 'ct_script_google_map' );
			wp_enqueue_script( 'ct_script_map' );
			wp_enqueue_script( 'ct_script_infobox' );
		}
		...
		//wp_register_style( 'ct_tab_home', CT_TEMPLATE_DIRECTORY_URI . '/css/tabs_home.css' );
		//wp_enqueue_style( 'ct_tab_home');
		//wp_register_script( 'ct_tab_home_js', CT_TEMPLATE_DIRECTORY_URI . '/js/tabs.js', array(), '', true );
		//wp_enqueue_script( 'ct_tab_home_js' );
	}
}
...
if ( ! function_exists( 'ct_register_sidebar' ) ) {
	function ct_register_sidebar() {
		...
		$args = array(
			'name'          => esc_html__( 'Cruise Sidebar', 'citytours' ),
			'id'            => 'sidebar-cruise',
		...
		$args = array(
			'name'          => esc_html__( 'Confirmation page sidebar', 'citytours' ),
		...
		$args = array(
			'name'          => esc_html__( 'Transfer sidebar', 'citytours' ),
			'id'            => 'sidebar-transfer',
			'description'   => '',
			'class'         => '',
			'before_widget' => '<div id="%1$s" class="widget %2$s">',
		...
		$args = array(
			'name'          => esc_html__( 'Tour Type Taxonomi sidebar', 'citytours' ),
			'id'            => 'sidebar-tour_type',
			'description'   => '',
			'class'         => '',
			'before_widget' => '<div id="%1$s" class="widget %2$s">',
			'after_widget'  => '</div><hr>',
		...
		$args = array(
			'name'          => esc_html__( 'Restaurant sidebar', 'citytours' ),
		...
		$args = array(
			'name'          => esc_html__( 'Sight sidebar', 'citytours' ),
			'id'            => 'sidebar-hanoi-sight',
			'description'   => '',
			'class'         => '',
			'before_widget' => '<div id="%1$s" class="widget %2$s">',
			'after_widget'  => '</div><hr>',
		...
		$args = array(
			'name'          => esc_html__( 'Golf Course Sidebar', 'citytours' ),
			'id'            => 'golf_course_sidebar',
			'description'   => '',
			'class'         => '',
			'before_widget' => '<div id="%1$s" class="small-box %2$s">',
			'after_widget'  => '</div>',
		...
		$args = array(
			'name'          => esc_html__( 'Golf Course Archive Sidebar', 'citytours' ),
			'id'            => 'golf_course_archive_sidebar',
			'description'   => '',
			'class'         => '',
			'before_widget' => '<div id="%1$s" class="small-box %2$s">',
			'after_widget'  => '</div>',
	...
	function ct_breadcrumbs() {
		...
		if ( is_home() ) {}
			...
			if( is_single() ) {
				...
				} elseif ( ( $post->post_type == 'cruise' ) ) {
					
					echo '<li class="active"><a href="/cruise" title="Halong Bay Cruises">Halong Bay Cruises</a></li>';
					echo '<li class="active">' . esc_html( get_the_title() ) . '</li>';
				} elseif ( ( $post->post_type == 'hotel' ) ) {
					
					echo '<li class="active"><a href="/hotel" title="Hotel">Hotels</a></li>';
					echo '<li class="active">' . esc_html( get_the_title() ) . '</li>';
				} elseif ( ( $post->post_type == 'tour' ) ) {
					
					echo '<li class="active"><a href="/tour" title="Tour">Hanoi Tours</a></li>';
					echo '<li class="active">' . esc_html( get_the_title() ) . '</li>';
				} elseif ( ( $post->post_type == 'transfer' ) ) {
					
					echo '<li class="active"><a href="/transfer" title="Hanoi Transfer Services">Hanoi Transfer Services</a></li>';
					echo '<li class="active">' . esc_html( get_the_title() ) . '</li>';	
				}  elseif ( ( $post->post_type == 'restaurant' ) ) {
					
					echo '<li class="active"><a href="/restaurant" title="Hanoi Restaurants">Hanoi Restaurants</a></li>';
					echo '<li class="active">' . esc_html( get_the_title() ) . '</li>';	
				} elseif ( ( $post->post_type == 'hanoi_sight' ) ) {
					
					echo '<li class="active"><a href="/hanoi_sight" title="Hanoi Sightseeing">Hanoi Sightseeing</a></li>';
					echo '<li class="active">' . esc_html( get_the_title() ) . '</li>';	
				} else {
